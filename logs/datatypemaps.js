...
function mapIntegerToString(source,target) { 
 ...
 function mapDateToAge(source,target) {
target.setValue(getAge(source.getValue()));
}
...
//			familyTarget.addValue(familySource.getValue(i));
	...
	target.setValue('value', source.getValue());
}function mapFHIR_markdownToString(source,target) {}function checkFilter(target,properties) { if (properties.containsKey('VALUESET')) {     if (properties.get('VALUESET').containsKey(target.value())) {                  return true;     } }   return false } 